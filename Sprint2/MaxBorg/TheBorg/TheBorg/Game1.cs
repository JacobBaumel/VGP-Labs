...
namespace TheBorg {
    ...
    public class Game1 : Microsoft.Xna.Framework.Game {
        ...
        Texture2D cube;
        Texture2D explosion;
        SoundEffect phasorSound;
        SoundEffect torpedoSound;
        SoundEffect borgTorpedoSound;
        ...
        Buttons[] gpadButtons;
        Color[] phasorColors;
        Rectangle[] phasorRects;
        Random r;
        KeyboardState prev;
        MouseState prevMouse;
        GamePadState prevGame;
        ...
        protected override void Initialize() {
            ...
            gpadButtons = new Buttons[] { Buttons.DPadUp, Buttons.DPadLeft, Buttons.DPadDown, Buttons.DPadRight, Buttons.B, Buttons.Y, Buttons.A, Buttons.X };
            phasorColors = new Color[] { Color.Red, Color.Yellow, Color.Green, Color.Blue };
            phasorRects = new Rectangle[] { new Rectangle(395, 0, 10, 240), new Rectangle(0, 235, 400, 10), new Rectangle(395, 250, 10, 250), new Rectangle(400, 235, 400, 10) };
            r = new Random();
            isDestroyed = false;
            isFiringBack = false;
            ...
            timeAtLaunch = 0;
            isExploding = false;
            prevGame = GamePad.GetState(PlayerIndex.One);
            phasorPower = 0;
            drawPhasor = false;
            borgPos = 0;
        }
        ...
        protected override void LoadContent() {
            ...
            phasorSound = Content.Load<SoundEffect>("phasorSound");
            torpedoSound = Content.Load<SoundEffect>("torpedoSound");
            borgTorpedoSound = Content.Load<SoundEffect>("borgTorpedoSound");
        ...
        protected override void Update(GameTime gameTime) {
            ...
            if(drawPhasor) {
                timeAtLaunch++;
                if(timeAtLaunch / 60 > phasorSound.Duration.TotalSeconds) {
                    drawPhasor = false;
                }
            }
            ...
            if(!isTorpeding && !drawPhasor) {
                ...
                for(int i = 0; i < gpadButtons.Length; i++) { 
                    if(isGamepadEvent(g, gpadButtons[i])) {
                        if (i <= 3)
                        {
                            torpedoSelect = i;
                            setPos();
                        ...
                        else phasorPower = i - 4;
                    }
                }

                if(hasClicked(m) || g.Triggers.Left > 0.5f) {
                    setPos();
                    fire();
                }

                if (g.Triggers.Right > 0.5f) {
                    phasorSound.Play();
                    drawPhasor = true;
                    timeAtLaunch = 0;
                }
            ...
            if (drawPhasor && torpedoSelect == borgPos) isDestroyed = true;
            ...
            selectedMj = MathHelper.Clamp((float) (selectedMj + g.ThumbSticks.Left.Y), 0, 9);
            propulsion = MathHelper.Clamp((float)(propulsion + g.ThumbSticks.Left.X), 1, 9);
        ...
        bool isGamepadEvent(GamePadState state, Buttons b) {
            return state.IsButtonDown(b) && prevGame.IsButtonUp(b);
        }
        ...
        protected override void Draw(GameTime gameTime) { 
            ...
            if(!isExploding)
                ...
                spriteBatch.Draw(explosion, torpedoDrawRect, null, Color.White, torpedoRotation, Vector2.Zero, SpriteEffects.None, 1.0f);
            spriteBatch.Draw(turret, turretRect, Color.White);
            if(isHit) spriteBatch.Draw(explosion, turretExplosion, Color.White);
            if(drawPhasor) spriteBatch.Draw(baseRect, phasorRects[torpedoSelect], phasorColors[phasorPower]);
            ...
            spriteBatch.DrawString(font, "" + ((int) selectedMj) + " MJ", new Vector2(370, 235), Color.Black);
            ...
            spriteBatch.DrawString(font, "Selected Power: " + ((int) selectedMj), barSelectedPowerText, Color.Black);
            spriteBatch.DrawString(font, "Selected Propulsion: " + ((int) propulsion), barSelectedPropulsionText, Color.Black);
            spriteBatch.End();
